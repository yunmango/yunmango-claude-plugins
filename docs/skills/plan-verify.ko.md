...
```text
1. 사용자          ── /plan-verify ──►  팀 리드
2. 팀 리드         ── 계획 ──────►      Claude Planner
3. Claude Planner  ── 계획 ──────►      팀 리드
4. 팀 리드         ── 검증 ──────►      Codex Verifier   (read-only)
5. Codex Verifier  ── verdict + findings ──►  팀 리드
6. 팀 리드         (triage + 수정, PASS 아니면)
7. 팀 리드         ── 최종 계획 ──►     사용자
```
